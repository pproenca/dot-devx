...
install_deps() {
  ...
  brew install zoxide fzf fd bat tmux starship git
}
...
setup_git() {
  ...
  read -r -p "Git username [pproenca]: " git_user
  git_user="${git_user:-pproenca}"

  read -r -p "Git email [8202400+pproenca@users.noreply.github.com]: " git_email
  git_email="${git_email:-8202400+pproenca@users.noreply.github.com}"

  read -r -p "Use 1Password SSH signing? [y/N]: " use_1pass
